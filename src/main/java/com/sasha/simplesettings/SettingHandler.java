package com.sasha.simplesettings;
...
public class SettingHandler {
    ...
    public SettingHandler(String fileNameToWriteTo) {
            ...
            try {
                ...
                for (Field declaredField : object.getClass().getSuperclass().getDeclaredFields()) {
                    ...
                    if (!yml.exists(target)) {
                        yml.set(target, declaredField.get(object) == null ? "[no default]" : declaredField.get(object));
                        declaredField.set(object, declaredField.get(object) == null ? "[no default]" : declaredField.get(object));
                        continue;
                    }
                    if (declaredField.getType() == float.class) {
                        declaredField.set(object, yml.getFloat(target));
                    } else {
    ...
    private File setupFile() throws IOException {
        File file = new File(this.fileNameToWriteTo + ".yml");
        if (!file.exists()) {
            file.createNewFile();
        }
        return file;
